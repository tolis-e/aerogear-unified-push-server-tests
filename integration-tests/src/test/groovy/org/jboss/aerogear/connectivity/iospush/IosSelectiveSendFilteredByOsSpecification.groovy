...
@Mixin([AuthenticationUtils, PushApplicationUtils, AndroidVariantUtils,
	...
	iOSVariantUtils])
class IosSelectiveSendFilteredByOsSpecification extends Specification {
	...
	def static WebArchive "create deployment"() {
		Deployments.customUnifiedPushServerWithClasses(IosSelectiveSendFilteredByOsSpecification.class)
	}
	...
	def "Register an Android Variant"() {
		...
		then: "Push Application id are not null"
		pushApplicationId != null
	...
	def "Verify that the inactive tokens were deleted"() {
		...
		deviceTokens != null && !deviceTokens.contains(IOS_DEVICE_TOKEN)&& !deviceTokens.contains(IOS_DEVICE_TOKEN_2)
	}
}
